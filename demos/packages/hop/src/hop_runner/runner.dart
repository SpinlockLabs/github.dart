...
class Runner {
  ...
  static Future<RunResult> run(HopConfig config,
      {Level printAtLogLevel, bool throwTaskExceptions: false}) {
    ...
    if (config.argResults.command != null) {
      ...
      return Future.forEach(tasks.keys, (String subTaskName) {
        ...
        return _runNamedTask(subTaskName, task, args, printAtLogLevel, config,
            throwTaskExceptions).then((RunResult rr) {
          finalResult = rr;
        });
      }).then((_) {
  ...
  static Future<RunResult> runShell(List<String> mainArgs,
    ...
    final _ConsolePrinter printer = new _ConsolePrinter(
        Zone.current.print, useColor, minLogLevel, prefixEnabled);
