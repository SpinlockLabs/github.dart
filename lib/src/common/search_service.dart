...
class SearchService extends Service {
  ...
  Stream<Repository> repositories(String query, {String? sort, int pages = 2}) {
    ...
    PaginationHelper(github)
      ...
      final items = input['items'] as List<Map<String, dynamic>>;

      items.map(Repository.fromJson).forEach(controller.add);
  ...
  Stream<Issue> issues(String query, {String? sort, int pages = 2}) {
    ...
    PaginationHelper(github)
      ...
      items.map(Issue.fromJson).forEach(controller.add);
    }).onDone(controller.close);
  ...
  Stream<User> users(
    ...
    PaginationHelper(github)
      ...
      items.map(User.fromJson).forEach(controller.add);
    }).onDone(controller.close);
