...
class SearchService extends Service {
  ...
  Stream<Repository> repositories(String query, {String sort, int pages: 2}) {
    ...
    new PaginationHelper(_github).fetchStreamed("GET", "/search/repositories", params: params, pages: pages).listen((response) {
  ...
  Stream<User> users(String query, {String sort, int pages: 2, int perPage: 30}) {
    ...
    new PaginationHelper(_github).fetchStreamed("GET", "/search/users", params: params, pages: pages).listen((response) {
      if (response.statusCode == 403 && response.body.contains("rate limit") && isFirst) {
        throw new RateLimitHit(_github);
      }
