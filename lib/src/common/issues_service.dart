...
class IssuesService extends Service {
  ...
  Stream<Issue> listAll(
      ...
      int perPage,
      List<String> labels}) {
    return _listIssues("/issues", milestoneNumber, state, direction, sort, since, perPage, labels);
  }
  ...
  Stream<Issue> listByUser(
      ...
      int perPage,
      List<String> labels}) {
    return _listIssues("/user/issues", milestoneNumber, state, direction, sort, since, perPage, labels);
  }
  ...
  Stream<Issue> listByOrg(String org,
      ...
      int perPage,
      List<String> labels}) {
    return _listIssues("/orgs/$org/issues", milestoneNumber, state, direction, sort, since, perPage, labels);
  }
  ...
  Stream<Issue> listByRepo(RepositorySlug slug,
      ...
      List<String> labels}) {
    return _listIssues(
        "/repos/${slug.fullName}/issues", milestoneNumber, state, direction, sort, since, perPage, labels);
  }

  Stream<Issue> _listIssues(String pathSegment, int milestoneNumber, String state, String direction, String sort,
      DateTime since, int perPage, List<String> labels) {
    ...
    return PaginationHelper(_github).objects("GET", pathSegment, Issue.fromJSON, params: params);
  }
  ...
  Future<Issue> edit(RepositorySlug slug, int issueNumber, IssueRequest issue) {
    return _github
        .request("PATCH", '/repos/${slug.fullName}/issues/$issueNumber', body: issue.toJSON())
        .then((response) {
      return Issue.fromJSON(jsonDecode(response.body) as Map<String, dynamic>) as Future<Issue>;
    });
  }
  ...
  Future<Issue> get(RepositorySlug slug, int issueNumber) =>
      _github.getJSON("/repos/${slug.fullName}/issues/$issueNumber", convert: Issue.fromJSON);
  ...
  Future<Issue> create(RepositorySlug slug, IssueRequest issue) async {
    var response = await _github.request("POST", '/repos/${slug.fullName}/issues', body: issue.toJSON());
  ...
  Stream<User> listAssignees(RepositorySlug slug) {
    return PaginationHelper(_github).objects("GET", "/repos/${slug.fullName}/assignees", User.fromJson);
  }
  ...
  Stream<IssueComment> listCommentsByIssue(RepositorySlug slug, int issueNumber) {
    return PaginationHelper(_github)
        .objects('GET', '/repos/${slug.fullName}/issues/$issueNumber/comments', IssueComment.fromJSON);
  }
  ...
  Stream<IssueComment> listCommentsByRepo(RepositorySlug slug) {
    return PaginationHelper(_github).objects('GET', '/repos/${slug.fullName}/issues/comments', IssueComment.fromJSON);
  }
  ...
  Future<IssueComment> getComment(RepositorySlug slug, int id) =>
      _github.getJSON("/repos/${slug.fullName}/issues/comments/$id", convert: IssueComment.fromJSON);

  /// Creates a new comment on the specified issue
  ///
  /// API docs: https://developer.github.com/v3/issues/comments/#create-a-comment
  Future<IssueComment> createComment(RepositorySlug slug, int issueNumber, String body) {
    var it = jsonEncode({"body": body});
    return _github.postJSON('/repos/${slug.fullName}/issues/$issueNumber/comments',
        body: it, convert: IssueComment.fromJSON, statusCode: StatusCodes.CREATED);
  }
  ...
  Stream<IssueLabel> listLabels(RepositorySlug slug) {
    return PaginationHelper(_github).objects("GET", "/repos/${slug.fullName}/labels", IssueLabel.fromJSON);
  }
  ...
  Future<IssueLabel> getLabel(RepositorySlug slug, String name) =>
      _github.getJSON("/repos/${slug.fullName}/labels/$name", convert: IssueLabel.fromJSON, statusCode: StatusCodes.OK);
  ...
  Future<IssueLabel> createLabel(RepositorySlug slug, String name, String color) {
    return _github.postJSON("/repos/${slug.fullName}/labels",
        body: jsonEncode({"name": name, "color": color}), convert: IssueLabel.fromJSON);
  }
  ...
  Future<bool> deleteLabel(RepositorySlug slug, String name) async {
    var response = await _github.request("DELETE", "/repos/${slug.fullName}/labels/$name");
  ...
  Stream<IssueLabel> listLabelsByIssue(RepositorySlug slug, int issueNumber) {
    return PaginationHelper(_github)
        .objects("GET", "/repos/${slug.fullName}/issues/$issueNumber/labels", IssueLabel.fromJSON);
  }

  /// Adds labels to an issue.
  ///
  /// API docs: https://developer.github.com/v3/issues/labels/#add-labels-to-an-issue
  Future<List<IssueLabel>> addLabelsToIssue(RepositorySlug slug, int issueNumber, List<String> labels) {
    return _github.postJSON<List<dynamic>, List<IssueLabel>>(
      "/repos/${slug.fullName}/issues/$issueNumber/labels",
      body: jsonEncode(labels),
      convert: (input) => input.cast<Map<String, dynamic>>().map(IssueLabel.fromJSON).toList(),
    );
  }
  ...
  Future<List<IssueLabel>> replaceLabelsForIssue(RepositorySlug slug, int issueNumber, List<String> labels) {
    return _github
        .request("PUT", "/repos/${slug.fullName}/issues/$issueNumber/labels", body: jsonEncode(labels))
        .then((response) {
      return jsonDecode(response.body).map((Map<String, dynamic> it) => IssueLabel.fromJSON(it));
    });
  }
  ...
  Future<bool> removeLabelForIssue(RepositorySlug slug, int issueNumber, String label) async {
    var response = await _github.request("DELETE", "/repos/${slug.fullName}/issues/$issueNumber/labels/$label");
  ...
  Stream<Milestone> listMilestones(RepositorySlug slug) {
    return PaginationHelper(_github).objects("GET", "/repos/${slug.fullName}/milestones", Milestone.fromJSON);
  }
