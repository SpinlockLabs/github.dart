...
class PullRequestsService extends Service {
  ...
  Future<PullRequest> get(RepositorySlug slug, int number) {
    return _github.getJSON("/repos/${slug.fullName}/pulls/${number}", convert: PullRequest.fromJSON, statusCode: StatusCodes.OK);
  }
  
  /// Creates a Pull Request based on the given [request].
  /// 
  /// API docs: https://developer.github.com/v3/pulls/#create-a-pull-request
  Future<PullRequestInformation> create(RepositorySlug slug, CreateRelease request) {
  ...
  Future<PullRequest> edit(RepositorySlug slug, int number, {String title, 
    ...
    return _github.request("POST", '/repos/${slug.fullName}/pulls/${number}', 
        body: JSON.encode(map)).then((response) {
  ...
  Future<PullRequestMerge> merge(RepositorySlug slug, int number, {String message}) {
    var json = {};
    ...
    return _github.request("PUT", "/repos/${slug.fullName}/pulls/${number}/merge", 
        body: JSON.encode(json)).then((response) {
  ...
  Stream<IssueComment> listCommentsByPullRequest(RepositorySlug slug, int number) {
    return new PaginationHelper(_github).objects("GET", 
        "/repos/${slug.fullName}/pulls/${number}/comments", IssueComment.fromJSON);
  }
  ...
  Stream<IssueComment> listComments(RepositorySlug slug) {
    return new PaginationHelper(_github).objects("GET", 
        "/repos/${slug.fullName}/pulls/comments", IssueComment.fromJSON);
  }
  ...
  Future<IssueComment> createComment(RepositorySlug slug, int number, CreatePullRequestComment comment) {
    return _github.postJSON('/repos/${slug.fullName}/pulls/${number}/comments', 
        body: comment.toJSON(), convert: IssueComment.fromJSON, statusCode: 201);
  }
