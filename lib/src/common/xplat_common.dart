...
///
/// If in a browser context, it will check sessionStorage (and optionally query strings
/// if [allowQueryAuth] is explicitly set to true, which is deprecated).
///
...
Authentication? findAuthenticationInMap(Map<dynamic, dynamic> map) {
  for (final key in COMMON_GITHUB_TOKEN_ENV_KEYS) {
    if (map.containsKey(key)) {
      return Authentication.withToken(map[key] as String?);
    }
    if (map['GITHUB_USERNAME'] is String && map['GITHUB_PASSWORD'] is String) {
      return Authentication.basic(
          map['GITHUB_USERNAME'] as String?, map['GITHUB_PASSWORD'] as String?);
    }
  }
  return null;
}
