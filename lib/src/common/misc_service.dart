...
class MiscService extends Service {
  ...
  Future<GitignoreTemplate> getGitignoreTemplate(String name) {
    return _github.getJSON("/gitignore/templates/${name}", convert: GitignoreTemplate.fromJSON);
  }
  ...
  Future<String> renderMarkdown(String input, {String mode: "markdown", String context}) {
    return _github.request("POST", "/markdown", body: JSON.encode({
      "text": input,
      "mode": mode,
      "context": context
    })).then((response) {
  ...
  Stream<Octocat> listOctodex({bool cors: false}) {
    ...
    _github.client.request(new http.Request("${cors ? "http://www.corsproxy.com/" : "http://"}${u}")).then((response) {
